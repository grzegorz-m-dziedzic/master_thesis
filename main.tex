...
\section{Cel pracy}
Opracowanie informatycznego systemu wspomagania podejmowania decyzji w rozmieszczeniu zraszaczy wodnych na zadanej powierzchni oraz poprowadzeniu rur pomiędzy nimi, a w szczególności zaproponowanie oraz przebadanie wielokryterialnych algorytmów genetycznych w kontekście następujących kryteriów:\\
\begin{enumerate}
	\item Minimalizacja obszaru nienawodnionego.
	\item Minimalizacja kosztu zraszaczy wodnych\\
\end{enumerate}
, przy zdefiniowanych ograniczeniach wymuszających minimalizację nawodnienia obszaru znajdującego się poza zaznaczonym terenem oraz minimalizację wielkości obszarów nawodnionych nadmiernie.
...
Na podstawie funkcji obrazującej wielkość obszaru nienawodnionego \eqref{eq:simple_f1} sformułowano bardziej szczegółową funkcję biorącą pod uwagę ograniczenia, o których mowa była wcześniej: minimalizację nawodnienia obszaru znajdującego się poza wyznaczonym terenem \eqref{eq:constraint_out_of_bounds} oraz minimalizację wielkości obszarów nawodnionych nadmiernie \eqref{eq:constraint_overrirr}. Wpływ ograniczeń został wprowadzony w formie funkcji kary sumującej stopień ich naruszenia\eqref{eq:constraint_sum}.
...
Po uwzględnieniu przedstawionych ograniczeń funkcja oceniająca stopień nawodnienia terenu wygląda w sposób przedstawiony na równaniu \eqref{eq:f_one}.
...
Dla danych $M, P$ oraz z uwzględnieniem zdefiniowanych ograniczeń \eqref{eq:constraint_out_of_bounds}, \eqref{eq:constraint_overrirr} znajdź wektor $S^{*}$ minimalizujący wielkość obszaru nienawodnionego oraz minimalizujący koszt danego rozwiązania \eqref{eq:final_model}.
...
\end{enumerate}
Aby skorzystać z powyższych metryk konieczne jest znalezienie prawdziwego frontu Pareto. Na potrzeby badań został on wygenerowany poprzez wybranie niezdominowanych rozwiązań ze skumulowanej puli rozwiązań powstałej na skutek uruchomienia każdego algorytmu 20 razy z wykorzystaniem 10 tysięcy generacji. Rezultaty każdej symulacji były dodawane do puli po czym zredukowane do rozwiązań niezdominowanych.

Do badań przygotowane zostały trzy przypadki testowe, a więc trzy różne obszary, które mają zostać nawodnione. Zostały one dobrane pod względem wielkości oraz kształtu, tak aby jak najlepiej zobrazować wpływ ograniczeń na działanie algorytmów.

W kolejnym podrozdziale zilustrowane i opisane zostaną rezultaty przeprowadzonych badań. Dla danego przypadku testowego każdy algorytm uruchamiany był dziesięć razy, tworząc pulę rozwiązań, z której wybierany były rozwiązania niezdominowane zilustrowane na wykresach. Algorytmy uruchamiane były z ustawieniami opisanymi w podrozdziale mówiącym o implementacji algorytmów genetycznych.

Funkcje, których wartości przedstawione zostały na wykresach wyglądają następująco:
\begin{equation}
	\begin{split}
		&F_{1}(S) = \sum_{m=1}^{M}\sum_{n=1}^{N} p(m,n) \cdot (1 - irr(m,n,S)) \\ +& 
		\dfrac{\sum_{m=1}^{M}\sum_{n=1}^{N} (1 - p(m,n)) \cdot irr(m,n,S) \cdot 100}{\sum_{m=1}^{M}\sum_{n=1}^{N} p(m,n)} \\ +& \dfrac{\sum_{m=1}^{M}\sum_{n=1}^{N} p(m,n) \cdot ovrirr(m,n,S) \cdot 100}{\sum_{m=1}^{M}\sum_{n=1}^{N} p(m,n)}\\\\
		&F_{2}(S) = \sum_{i=1}^{S} C(t_{i})
	\end{split}
\end{equation}
Dokładny opis przedstawionych funkcji został zaprezentowany w rozdziale poświęconym modelowi matematycznemu.
...
Prawdopodobieństwo mutacji              & 0.05                      & 0.005, 0.01, 0.02, ... 0.1, 0.2 ... 0.5 \\ \hline
...
\begin{figure}[!htb]
	\makebox[\textwidth]{\includegraphics[ clip, scale=0.55]{small_area}}
	\centering
...
Rozwiązania z 1000. generacji znajdują się zdecydowanie najbliżej rozwiązań optymalnych. Ciekawą obserwacją jest to, że wartość metryki $GD$ dla algorytmu SPEA w ostatnim pokoleniu jest gorsza niż algorytmu NSGA-II dla pokolenia 500. SPEA prezentuje się jednak lepiej, jeśli chodzi o wartość metryki definiującej współczynnik błędu - w porównaniu do liczby wszystkich rozwiązań niezdominowanych algorytm SPEA znalazł więcej rozwiązań leżących na froncie Pareto. W porównaniu do pokolenia 500. algorytm NSGA-II znacząco powiększył swoją przewagę, jeśli chodzi o długość wygenerowanego frontu.

Podsumowując, z pierwszym przypadkiem testowym oba algorytmy poradziły sobie dobrze. NSGA-II znalazł więcej rozwiązań niezdominowanych. Co więcej rozwiązania te znajdują się bliżej frontu Pareto niż rozwiązania znalezione przez algorytm SPEA. Poza tym NSGA-II wygenerował znacząco dłuższy front.

Z drugiej strony algorytm SPEA znalazł więcej rozwiązań znajdujących się na froncie Pareto co określa metryka $ER$. Ponadto rozwiązania znalezione przez SPEA charakteryzują się większą równomiernością rozkładu, choć może to być spowodowane mniejszą liczbą rozwiązań branych pod uwagę.
...
\begin{figure}[!htb]
	\makebox[\textwidth]{\includegraphics[ clip, scale=0.55]{medium_area}}
	\centering
...
\end{figure*}
Analizując przedstawione wykresy można łatwo zauważyć, że algorytm NSGA-II w każdym z przedstawionych pokoleń wygenerował rozwiązania lepsze od tych znalezionych przez SPEA. Jest to dobrze widoczne na wykresie prezentującym wyniku po 500. pokoleniu. Dla dużych wartości funkcji $F_{2}$, gdzie front Pareto jeszcze nie został osiągnięty, żadne rozwiązanie znalezione przez NSGA-II nie jest zdominowane przez którekolwiek rozwiązanie wygenerowane przez SPEA. Taka sytuacja utrzymuje się, aż do końca symulacji.
\begin{figure*}\centering
...
Podsumowując, z drugim przypadkiem testowym zdecydowanie lepiej poradził sobie algorytm NSGA-II, co dobrze widoczne jest na przedstawionych wykresach.
...
\begin{figure}[!htb]
	\makebox[\textwidth]{\includegraphics[ clip, scale=0.55]{big_area}}
	\centering
...
Ostatnim przypadek testowy był reprezentowany przez największy obszar. W tym przypadku żadnemu algorytmowi przez 5000 generacji nie udało się znaleźć rozwiązań rozciągających się na całej długości frontu Pareto. Podobnie jak w poprzednim przypadku dłuższy front wygenerował algorytm SPEA. Nie przekłada się to jednak na ogólną jakość rozwiązań. NSGA-II znalazł rozwiązania cechujące się lepszym wartościami dla wszystkich pozostałych metryk.
...
Ponadto wybrano oraz porównano ze sobą dwa wielokryterialne algorytmy genetyczne. Do badań wykorzystano trzy przypadki testowe oraz zdefiniowano cztery metryki pomagające w ocenie wygenerowanych przez algorytmy rozwiązań. Z przeprowadzonych badań wynika, że dla problemów reprezentowanych przez stosunkowo małe obszary równie dobrze spisują się oba algorytmy. Biorąc jednak pod uwagę ich złożoność czasową, lepszą opcją w przypadku takich problemów może okazać się algorytm SPEA, który charakteryzuje się szybszym działaniem. Dla trudniejszych przypadków testowych zdecydowanie lepiej wypada algorytm NSGA-II, którego rozwiązania cechują się lepszymi wartościami dla wszystkich zdefiniowanych metryk.

W opracowanym systemie wspomagania decyzji zdecydowano się na wykorzystanie algorytmu NSGA-II, biorąc pod uwagę wyniki przeprowadzonych badań, zarówno pod względem liczby iteracji jak i złożoności czasowej.
...
Kolejnym elementem wartym modyfikacji jest zdefiniowanie bardziej dokładnego modelu matematycznego. W tym momencie brane są pod uwagę jedynie zraszacze dookolne co nakłada dodatkowe ograniczenia na znalezione rozwiązania. Przykładem jest chociażby fakt, że dany obszar nie może zostać w 100\% nawodniony bez naruszenia ograniczeń mówiących o nie podlewaniu obszaru poza wyznaczonym przez użytkownika. Problem ten może zostać zlikwidowany poprzez dodanie dodatkowych cech do rozmieszczonych zraszaczy, takich jak kąt rozrzutu wody oraz kierunek rozrzutu.
